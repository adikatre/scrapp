...
export function ScanView({
  ...
  const renderCameraView = () => {
    ...
    return (
      <Webcam
        className={`absolute inset-0 h-full w-full object-cover ${isCapturing ? "brightness-150" : ""}`}
        ref={cameraRef}
      ...
      <div className="absolute bottom-10 left-1/2 flex -translate-x-1/2 items-center gap-8">
        ...
        <button
          ...
          className={`
            ...
            ${isCapturing ? "scale-95 bg-primary animate-pulse" : "hover:scale-110 active:scale-95"}
            shadow-lg hover:shadow-xl
            disabled:cursor-not-allowed
          `}>
          <div className="size-20 flex items-center justify-center">
            <CameraIcon
              className={`size-10 transition-colors ${isCapturing ? "text-white" : "text-gray-700"}`}
            />
          </div>
        </button>
  ...
  // Render camera frame corners
  const renderCorners = () => {
    if (capturedImage || activeTicket) return null;
    return (
  ...
  // Render capture flash
  const renderCaptureFlash = () => {
    if (!isCapturing) return null;
    return <div className="pointer-events-none absolute inset-0 bg-white/30 animate-pulse" />;
  };
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-black">
      ...
      {/* Top overlay bar - history + camera switch */}
