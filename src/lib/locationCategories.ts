...
export const CATEGORY_ACCENTS: Record<LocationCategoryKey, string> = {
  recycle: "bg-emerald-500/15 text-emerald-800 ring-emerald-700/30 dark:text-emerald-300",
  compost: "bg-lime-500/15 text-lime-800 ring-lime-700/30 dark:text-lime-300",
  e_waste: "bg-sky-500/15 text-sky-800 ring-sky-700/30 dark:text-sky-300",
  hazardous: "bg-rose-500/15 text-rose-800 ring-rose-700/30 dark:text-rose-300",
  donation: "bg-amber-500/15 text-amber-800 ring-amber-700/30 dark:text-amber-300",
  single_use: "bg-orange-500/15 text-orange-800 ring-orange-700/30 dark:text-orange-300",
  general_trash: "bg-zinc-500/15 text-zinc-800 ring-zinc-700/30 dark:text-zinc-300",
  city_infra: "bg-cyan-500/15 text-cyan-800 ring-cyan-700/30 dark:text-cyan-300",
  living_things: "bg-teal-500/15 text-teal-800 ring-teal-700/30 dark:text-teal-300"
};

export const BIN_ACCENTS: Record<string, string> = {
  "Blue Bin (Recycling)": "bg-blue-500/15 text-blue-800 ring-blue-700/30 dark:text-blue-300",
  "Green Bin (Organics)": "bg-green-500/15 text-green-800 ring-green-700/30 dark:text-green-300",
  "Gray Bin (Trash)": "bg-zinc-500/15 text-zinc-800 ring-zinc-700/30 dark:text-zinc-300",
  "Special Drop-off": "bg-amber-500/15 text-amber-800 ring-amber-700/30 dark:text-amber-300"
};

export function getCategoryAccent(key: string): string {
  if (key in CATEGORY_ACCENTS) return CATEGORY_ACCENTS[key as LocationCategoryKey];
  return CATEGORY_ACCENTS.recycle;
}
...
export function isLocationCategoryKey(value: string): value is LocationCategoryKey {
  return (LOCATION_CATEGORIES as ReadonlyArray<{ key: LocationCategoryKey }>).some(
    (c) => c.key === value
  );
}
...
import { buildLocationSearchHref } from "./locationSearchParams";
...
export function buildLocationsHref(
  ...
  return buildLocationSearchHref({
    category,
    item: itemName ?? undefined,
    queries: searchQueries ?? undefined,
    bin: getCurbsideBinInfo(bin) ? (bin ?? undefined) : undefined
  });
}
